...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let startButton = document.getElementById('startButton');
    let stopButton = document.getElementById('stopButton');
    let OpenNewWindow = document.getElementById("open");
    OpenNewWindow.addEventListener("click", () => {
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, { action: 'open' });
        });
    });
    ...
    // document.getElementById('showRectangleButton').addEventListener('click', () => {
    //     // Query the active tab and use the tabId to insert CSS and execute script
    //     chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
